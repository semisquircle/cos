...
MalaysianMadgwick::MalaysianMadgwick() {
	beta = 0.1f;
	q0 = 1.0f;
	q1 = 0.0f;
	q2 = 0.0f;
...
float MalaysianMadgwick::invSqrt(float x) {
	float halfx = 0.5f * x;
	union {
	float    f;
	uint32_t i;
...
bool MalaysianMadgwick::initQuat(float ax, float ay, float az, float mx, float my, float mz) {
...
void MalaysianMadgwick::vectorCross(float A[3], float B[3], float cross[3]) {
...
float MalaysianMadgwick::deltatUpdate() {
	Now = millis();
...
void MalaysianMadgwick::updateIMU(float gx, float gy, float gz, float ax, float ay, float az, float deltat) {
	float recipNorm;
	float s0, s1, s2, s3;
...
void MalaysianMadgwick::updateMARG(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz, float deltat) {
	float recipNorm;
	float s0, s1, s2, s3;
	float qDot1, qDot2, qDot3, qDot4;
	float hx, hy;
...
void MalaysianMadgwick::getQuaternions(float &qw, float &qx, float &qy, float &qz) {
	qw = q0;
	qx = q1;
	qy = q1;
	qz = q2;
...
void MalaysianMadgwick::getEulerAngles(float &yaw, float &pitch, float &roll) {
