...
void Madgwick2010::setSamplingRate(float rate) {
	deltat = 1.0f / rate;
}


void Madgwick2010::setGyroMeasError(float error) {
	gyroMeasError = error;
	beta = sqrt(3.0f / 4.0f) * gyroMeasError;
}


void Madgwick2010::setGyroDriftError(float error) {
	gyroMeasDrift = error;
	zeta = sqrt(3.0f / 4.0f) * gyroMeasDrift;
}


void Madgwick2010::updateIMU(float w_x, float w_y, float w_z, float a_x, float a_y, float a_z) {
...
void Madgwick2010::updateMARG(float w_x, float w_y, float w_z, float a_x, float a_y, float a_z, float m_x, float m_y, float m_z) {
	// local system variables
	float norm; // vector norm
...
void Madgwick2010::getQuaternions(float &q1, float &q2, float &q3, float &q4) {
	q1 = SEq_1;
	q2 = SEq_2;
	q3 = SEq_3;
	q4 = SEq_4;
...
void Madgwick2010::getEulerAngles(float &pitch, float &yaw, float &roll) {
	pitch = asin(-2.0f * (SEq_2 * SEq_4 - SEq_1 * SEq_3));
	yaw = atan2(SEq_2 * SEq_3 + SEq_1 * SEq_4, 0.5f - SEq_3 * SEq_3 - SEq_4 * SEq_4);
	roll = atan2(SEq_1 * SEq_2 + SEq_3 * SEq_4, 0.5f - SEq_2 * SEq_2 - SEq_3 * SEq_3);
}
